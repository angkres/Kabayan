...
public class GameEngine extends GameCore 
{
    ...
    public void draw(Graphics2D g) {
        ...
        // Enter finish game, state level = -1
            ...
            g.setColor(Color.YELLOW);
            g.drawString("SELAMAT TJOIIII!!!!!",320.0f,200.0f);
            g.drawString("SELAMAT TJOIIII!!!!!",320.0f,240.0f);
            g.drawString("SELAMAT TJOIIII!!!!!",320.0f,260.0f);
            g.setColor(Color.GREEN);
            g.drawString("Score: "+collectedStars,320.0f,300.0f);
            g.setColor(Color.WHITE);
            g.drawString("Press ESC for EXIT.",320.0f,20.0f);
        }
        // Enter game over, state level = -2
        else{
            g.setColor(Color.RED);
            g.drawString("Score: "+collectedStars,360.0f,180.0f);
            g.setColor(Color.WHITE);
            g.drawString("Press ESC for EXIT.",320.0f,20.0f);
        }
    } 
    ...
    public void checkPlayerCollision(Player player,
        ...
        if (collisionSprite instanceof PowerUp) {
            ...
            if (canKill) {
                ...
                if(numLives == 0) {
                    drawer = new TileMapDrawer();
                    drawer.setBackground(mapLoader.loadImage("backYouDied.jpg"));
    ...
    public void acquirePowerUp(PowerUp powerUp) {
        ...
        if (powerUp instanceof PowerUp.Star) {
            ...
            if (mapLoader.currentMap <= 4){     //on testing
                map = mapLoader.loadNextMap();
            }
            // finsih the game when player reaches the end of level (level 4)
                ...
                drawer = new TileMapDrawer();
                drawer.setBackground(mapLoader.loadImage("backFinish.jpg"));
            } 
        }
    }
}